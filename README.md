...
Code logic explanation:
  ...
  The Arduino program is used to count how many obstacles are detected by an IR sensor and show the count on an LCD display. First, the program sets up the LCD using the `LiquidCrystal_I2C` library and sets pin 9 as an input, where the IR sensor is connected. It also starts serial communication and displays the title “No.of Obstacles” on the first line of the LCD.

Inside the loop, the program keeps checking the sensor value using `digitalRead`. It uses two variables, `prev` and `current`, to store the previous and current readings of the sensor. This helps the program detect when a new object passes in front of the sensor. Normally, the sensor gives a value of 1 when there is no object and 0 when an object is detected. The program looks for a change from 1 to 0, which means a new obstacle has just appeared. When this change happens, the counter variable `sum` is increased by 1.
